...
class ExecutionTests(unittest.TestCase):
    ...
    def test_live_requires_both_independent_gates(self):
        for enabled, token in [(False, ""), (True, "wrong"), (False, LIVE_CONFIRMATION)]:
            result = execute_plan(
                self.plan, self.venue, mode=ExecutionMode.LIVE,
                live_enabled=enabled, confirmation=token,
            )
    ...
    def test_cancel_rejection_blocks_place(self):
        self.venue.cancel_ok = False
        self.assertEqual(self.live().status, "cancel_rejected")
        self.assertEqual([call[0] for call in self.venue.calls], ["cancel"])
